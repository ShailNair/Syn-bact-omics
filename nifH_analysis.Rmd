...
#(240, 380) is the range of targeted amplicon length. Change according to your amlicon length.

```{r}
seqtab <- seqtab[, nchar(colnames(seqtab)) %in% seq(240, 380)]
```
...
#note- first run "seqtab.nochim" with minFoldParentOverAbundance = 1 (default), see the percent of non chimeras. if a lot of reads are removed as chimeras and you are sure that your reads completely cleaned of primers then try increasing  minFoldParentOverAbundance = 2,4,6, or 8. Finally, if you are not satisfied with the results, you can always, start again making sure to use cleaned, quality reads.
```{r}
